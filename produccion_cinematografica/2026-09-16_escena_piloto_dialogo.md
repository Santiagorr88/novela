...
**Validado en la práctica (2026-09-16)**, ya no es solo teoría:

1. En el proyecto de Flow (ej. "CAP01"), pestaña **Caracteres**: los personajes ya están creados (Miguel, Gabriel, Lucifer, Pewter_Sentinel), con las variantes de Miguel (armadura agrietada, ala quemada) ya aprobadas — ver `../variantes_por_personaje/Miguel.md`.
2. Pestaña **Escenas** → nueva escena → referencia el personaje que toque en ese plano (solo el que habla/aparece; en shot/reverse-shot no hace falta meter a los dos personajes en el mismo plano).
3. Pega el prompt del plano (sección 4 de este documento).
4. **Modelo de vídeo: `Vídeo → Ingredientes → Omni 1.1 Flash`, 720p, 8s** — **no** "Veo 3.1 - Quality". Confirmado con el plano 5 real: `Omni 1.1 Flash` sí admite el personaje como Ingrediente (Veo 3.1 Quality daba error "No puedes usar ingredientes de personaje con este modelo"), genera **audio y lip-sync correctos**, y cuesta **12 puntos** por clip de 8s frente a los 100 puntos de Veo 3.1 Quality — más de 8 veces más barato. Guardar esto como el modelo por defecto para el resto del capítulo, no solo para esta escena.
   - Solo si algún plano concreto sale sin audio o con lip-sync raro con Omni 1.1 Flash, recurrir al plan B: generar antes una imagen fija con `Imagen → Ingredientes → Nano Banana Pro` (0 puntos), y luego `Vídeo → Fotogramas` (subiendo esa imagen) con `Veo 3.1 - Quality`.
5. **Revisa cada clip contra la ficha** antes de pasar al siguiente: ¿la cara/pelo/armadura coinciden con el canonical? ¿la voz suena "grave y serena" (Miguel) / "dulce y cálida" (Gabriel) como pide `personajes.md`? Si algo deriva, regenera ese plano repitiendo el personaje de referencia, no sigas adelante con uno que no cuadra.
6. **Descarga cada clip** y nómbralo `cap01_esc_dialogo_plano01.mp4`, `...plano02.mp4`, etc., siguiendo la numeración de la tabla — así al montar en DaVinci/Premiere el orden es evidente.
7. Cuando tengas los 18 descargados, pásamelos (o dime que están listos) y reviso el conjunto antes de pasar a generar el audio de las voces de Miguel/Gabriel (voces ya fichadas en `audiolibro_casting.json`: Miguel="Hernán", Gabriel="Enrique" — recordar la regla de `CLAUDE.md`: nada de audio real hasta que tú lo pidas y se calcule el coste antes) y montar la escena completa.

**Nota de coste actualizada**: con Omni 1.1 Flash a 12 puntos/clip, un capítulo de ~130 planos costaría ≈1.560 puntos — muy por debajo de los 25.000 puntos/mes del plan Ultra, y lejísimos de la estimación de la investigación general (que asumía "Quality" a 100 puntos/clip = ~13.000 puntos/capítulo). Actualiza la expectativa de cuántos capítulos caben en una cuota mensual — bastantes más de los que calculé antes de probarlo.
